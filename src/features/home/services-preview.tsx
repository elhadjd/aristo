        ...
        {services.slice(0, 6).map((service, index) => {
          ...
          return (
            <Reveal key={service.id} delay={index * 0.05}>
              <Link
                href={comingSoon ? "/financing" : `/services#${service.slug}`}
                className="relative block h-full rounded-2xl border border-border bg-card p-6 shadow-soft transition hover:-translate-y-1 hover:shadow-lift"
              >
                {comingSoon ? (
                  <span className="absolute right-4 top-4 rounded-full border border-border bg-muted-bg px-2.5 py-0.5 text-[11px] font-medium uppercase tracking-wide text-muted">
                    Coming soon
                  </span>
                ) : null}
